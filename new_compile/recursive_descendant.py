...
def simpleExprL(ts,p):
	if ts.peek() in p.predict(71):
		ExprR(ts,p)
		simpleExprLTail(ts,p)
	else:
		print("Syntax Error at 'simpleExprL'")
		sys.exit(0)

def simpleExprLTail(ts,p):
	if ts.peek() in p.predict(72):
		relationOperator(ts,p)
		ExprR(ts,p)
