...
def create_grammar() -> GrammarEx:
    ...
    G.add_nonterminal('ExprLPrime')# 41
    G.add_nonterminal('ExprR')     # 42
    G.add_nonterminal('ExprRPrime')# 43
    G.add_nonterminal('ExprM')     # 44
    G.add_nonterminal('Factor')    # 45
    G.add_nonterminal('OP')        # 46

    # Produções
    G.add_production('Prog', ['Decls', '$'])  # 47
    G.add_production('Decls', ['Decl', 'Decls'])  # 48
    G.add_production('Decls', ['Stmt', 'Decls'])  # 49
    G.add_production('Decls', [])  # 50
    G.add_production('Decl', ['let', 'id', 'colon', 'Type', 'DeclPrime'])  # 51
    G.add_production('Type', ['i32'])  # 52
    G.add_production('Type', ['f32'])  # 53
    G.add_production('Type', ['&str'])  # 54
    G.add_production('DeclPrime', ['semicolon'])  # 55
    G.add_production('DeclPrime', ['=', 'ExprL', 'semicolon'])  # 56
    G.add_production('Stmt', ['IfStmt'])  # 57
    G.add_production('Stmt', ['WhileStmt'])  # 58
    G.add_production('IfStmt', ['if', 'lparen', 'ExprL', 'rparen', 'lbrace', 'Decls', 'rbrace', 'ElsePart', 'endif'])  # 59
    G.add_production('ElsePart', ['else', 'lbrace', 'Decls', 'rbrace'])  # 60
    G.add_production('ElsePart', [])  # 61
    G.add_production('WhileStmt', ['while', 'lparen', 'ExprL', 'rparen', 'lbrace', 'Decls', 'rbrace', 'endwhile'])  # 62
    G.add_production('ExprL', ['!', 'ExprL', 'ExprLPrime'])  # 63
    G.add_production('ExprL', ['ExprR', 'ExprLPrime'])  # 64
    G.add_production('ExprLPrime', ['&&', 'ExprL'])  # 65
    G.add_production('ExprLPrime', ['||', 'ExprL'])  # 66
    # G.add_production('ExprLPrime', [])  # 67
    G.add_production('ExprR', ['ExprM', 'ExprRPrime'])  # 68
    G.add_production('ExprRPrime', ['OP', 'ExprM'])  # 69
    # G.add_production('ExprRPrime', [])  # 70
    G.add_production('ExprM', ['Factor', 'ExprRPrime'])  # 71
    G.add_production('Factor', ['id'])  # 72
    G.add_production('Factor', ['lparen', 'ExprL', 'rparen'])  # 73
    G.add_production('OP', ['plus'])  # 74
    G.add_production('OP', ['minus'])  # 75
    G.add_production('OP', ['times'])  # 76
    G.add_production('OP', ['divide'])  # 77
    G.add_production('OP', ['=='])  # 78
    G.add_production('OP', ['!='])  # 79
    G.add_production('OP', ['>'])  # 80
    G.add_production('OP', ['>='])  # 81
    G.add_production('OP', ['<'])  # 82
    G.add_production('OP', ['<='])  # 83
